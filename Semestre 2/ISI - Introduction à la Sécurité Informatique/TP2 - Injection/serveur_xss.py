...
class VulnerableApp(object):
    ...
    def index(self, **post):
        ...
        if cherrypy.request.method == "POST":
            ...
            insert_tuple_in_request = (html.escape(post["chaine"]) , cherrypy.request.remote.ip)
            print("req: [" + requete + "]", insert_tuple_in_request)
            cursor.execute(requete, insert_tuple_in_request)
            self.conn.commit()
